  ...
  function switchScreen(name) {
    ...
    if (name === 'home') { renderCustomVideos(); renderCustomFeedPosts(); renderHeroLesson(); renderDailyGoal(); }
    if (name === 'lessons') { renderCustomLessons(); renderLessonPath(); }
    if (name === 'profile') { syncAchievementsStrip(); renderHomeworkList(); renderSavedWords(); renderFriendsAndChat(); }
  ...
  function saveHangulWord() {
    ...
    const added = saveMyWord(word, '', '', '🌸', 'hangul');
    if (!added) { toast('Это слово уже сохранено 🌸'); return; }
    if (inp) inp.value = '';
    toast('🌸 «' + word + '» сохранено в словарик');
    recordHangulSave();
    renderSavedWords();
  }
  ...
  function getMyWords() {
    ...
    if (!Array.isArray(list)) {
      // Migrate legacy hangulWords storage (ko-only, no translation)
      const legacy = UStore.get('hangulWords', []);
  ...
  function saveMyWord(ko, ru, translit, emoji, source) {
    ...
    if (idx >= 0) {
      // Already saved — backfill missing fields if we have richer data now
  ...
  function toggleMyWord(ko, ru, translit, emoji, source, btn) {
    ...
    if (btn) {
      ...
      btn.innerHTML = saved
        ? '<i class="fa-solid fa-heart" style="color:#E07686;"></i>'
  ...
  function renderSavedWords() {
    ...
    const studyBtn = list.length >= 3
      ? `<button onclick="startMyWordsFlashcards()" class="btn btn-primary" style="padding:7px 14px; font-size:11.5px; flex-shrink:0;"><i class="fa-solid fa-graduation-cap" style="font-size:10px;"></i> Учить (${list.length})</button>`
      : '';
    slot.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;padding:0 2px;">
        <div style="font-size:10.5px;color:var(--soft);">Всего: <b style="color:var(--coral);">${list.length}</b></div>
        ${studyBtn}
      </div>
      <div style="display:grid; gap:6px;">
        ${list.map(w => `
          <div style="display:flex; align-items:center; gap:10px; background:rgba(255,255,255,.7); border:1px solid var(--line); border-radius:12px; padding:8px 12px;">
            <span style="font-size:18px; flex-shrink:0;">${w.emoji || '🌸'}</span>
            <div style="flex:1; min-width:0; cursor:pointer;" onclick="playSyllable('${String(w.ko).replace(/'/g, "\\'")}', this)">
            ...
            <button onclick="removeMyWord('${String(w.ko).replace(/'/g, "\\'")}')" aria-label="Удалить" style="background:none; border:none; color:var(--soft); cursor:pointer; font-size:18px; line-height:1; padding:4px 6px; flex-shrink:0;">×</button>
          </div>
        `).join('')}
      </div>`;
  }
  ...
  function renderLesson() {
    ...
    // Reuse existing page element so the slide-in animation doesn't replay on each step
    let m = document.getElementById('lesson-modal');
    const isNew = !m;
    if (isNew) {
      m = document.createElement('div');
      m.className = 'lesson-page';
      m.id = 'lesson-modal';
      document.body.appendChild(m);
    ...
    if (!lesson) {
      m.innerHTML = `
        <div class="lesson-page-body" style="text-align:center; justify-content:center;">
          <div style="font-size:64px;">🎉</div>
          <div class="display" style="font-size:28px; color:var(--berry); margin-top:14px;">Все уроки пройдены</div>
          <div style="font-size:13px; color:var(--soft); margin-top:8px;">Ждём новых уроков от Мади 🌸</div>
          <button onclick="closeLessonFlow()" class="btn btn-primary" style="margin-top:24px; max-width:280px;">Закрыть</button>
        </div>`;
      return;
    }
    ...
    // If page already exists, update only header + body so the wrapper isn't recreated
    let header = m.querySelector('.lesson-page-header');
    let bodyEl = m.querySelector('.lesson-page-body');
    if (!header || !bodyEl) {
      m.innerHTML = `
        <div class="lesson-page-header">
          <button onclick="closeLessonFlow()" aria-label="Закрыть" style="background:none; border:none; font-size:24px; color:var(--soft); cursor:pointer; line-height:1; padding:4px 8px;">×</button>
          <div class="lesson-segs" style="display:flex; gap:4px; flex:1;">${segs}</div>
          <div class="lesson-counter" style="font-size:11px; color: var(--soft); white-space:nowrap;">${lessonStage+1} / ${total}</div>
        </div>
        <div class="lesson-page-body">${body}</div>
      `;
    } else {
      const segsHost = header.querySelector('.lesson-segs');
      const counter  = header.querySelector('.lesson-counter');
      if (segsHost) segsHost.innerHTML = segs;
      if (counter)  counter.textContent = `${lessonStage+1} / ${total}`;
  ...
  function renderHomeworkList() {
    ...
    slot.innerHTML = LESSON_CATALOG.map(l => {
      const isDone = admin || completed.has(l.id);
      const tasks = (l.homework && l.homework.tasks ? l.homework.tasks.length : 0);
      const words = Array.isArray(l.vocab) ? l.vocab.length : 0;
      if (isDone) {
        return `
      ...
      return `
        <div class="card card-padded" style="display:flex; align-items:center; gap:12px; margin-bottom:10px; border-style:dashed; background:rgba(255,255,255,.5); cursor:not-allowed;">
          <div style="width:38px; height:38px; flex-shrink:0; border-radius:12px; background:rgba(92,42,51,.06); display:flex; align-items:center; justify-content:center; font-size:14px; color:rgba(92,42,51,.45);"><i class="fa-solid fa-lock"></i></div>
          ...
          <span class="chip" style="flex-shrink:0; font-size:10px; background:rgba(92,42,51,.06); color:var(--soft);">🔒</span>
        </div>`;
    }).join('');
  }
  ...
  function lessonQuizItem(it, pool) {
    ...
    // Korean column auto-widens to its content (white-space: nowrap),
    // font shrinks for longer verbs so 4-char words stay on one line.
    const koLen = (it.ko || '').replace(/\s/g, '').length;
    const koSize = koLen >= 4 ? '22px' : (koLen === 3 ? '26px' : '32px');
    return `
      <div class="card card-padded quiz-row" style="display:flex; align-items:center; gap:14px; padding:14px 16px;">
        <div class="ko" style="font-size:${koSize}; font-weight:800; color:var(--berry); flex-shrink:0; white-space:nowrap; line-height:1.1; min-width:64px; text-align:center;">${it.ko}</div>
        <div style="display:flex; gap:8px; flex:1; flex-wrap:wrap; min-width:0;">
          ${opts.map(o => `<button onclick="lessonQuizPick(this, '${o}', '${it.ru}')" class="btn btn-ghost" style="flex:1; min-width:70px; padding:10px 8px; font-size:12.5px; white-space:nowrap;">${o}</button>`).join('')}
        </div>
      </div>`;
  }
  function lessonQuizPick(btn, chosen, correct) {
    ...
    if (chosen === correct) {
      ...
      btn.style.background = 'rgba(132,196,116,.22)';
      btn.style.borderColor = '#4F7B43';
      btn.style.color = '#4F7B43';
      btn.classList.add('quiz-correct-pop');
      if (row) row.dataset.done = '1';
      lessonQuizCheck();
    } else {
      btn.style.background = '#FFEBEE';
      btn.style.borderColor = '#D8525F';
      btn.style.color = '#B33A4A';
      btn.classList.add('quiz-wrong-shake');
      setTimeout(() => {
        btn.style.background = '';
        btn.style.borderColor = '';
        btn.style.color = '';
        btn.classList.remove('quiz-wrong-shake');
      }, 700);
    }
  }
      ...
      <div class="modal-card">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:14px;">
          ...
          <div style="display:flex; gap:6px; align-items:center;">
            <button onclick="toggleMyWord('${card.front.replace(/'/g, "\\'")}', '${(card.meaning||'').replace(/'/g, "\\'")}', '${(card.translit||'').replace(/'/g, "\\'")}', '${(card.emoji||'').replace(/'/g, "\\'")}', 'flashcards', this)" aria-label="В словарик" title="Сохранить в Мои слова" style="background:rgba(255,255,255,.7); border:1px solid var(--line); width:34px; height:34px; border-radius:10px; cursor:pointer; font-size:14px; line-height:1; padding:0;">${heartIcon}</button>
            <div onclick="this.closest('.modal-bg').remove()" style="font-size:24px; line-height:1; color:var(--soft); cursor:pointer; padding:4px 8px;">×</div>
          </div>
        </div>
  ...
  function myName() { const u = Store.get('user'); return (u && u.name) || 'Без имени'; }
  ...
  async function sendFriendRequest(toUid, toName) {
    ...
    const me = myUid();
    if (!me || me === 'guest') { toast('Войди, чтобы добавлять друзей'); return; }
    if (me === toUid) { toast('Нельзя добавить себя 🌸'); return; }
    if (_sendingFriendReq.has(toUid)) return;       // Уже отправляем — игнор спама
    if (_outgoingCache[toUid])        { toast('Заявка уже отправлена ⏳'); return; }
    if (_friendsCache[toUid])         { toast('Уже в друзьях'); return; }
    ...
    try {
      ...
      if (existingFr.exists()) { toast('Уже в друзьях'); return; }
      const existingOut = await _db.ref(`users/${me}/outgoing/${toUid}`).once('value');
      if (existingOut.exists()) { toast('Заявка уже отправлена ⏳'); return; }
      ...
      toast('Заявка отправлена 🌸', 'var(--sage)');
    } catch (e) {
      console.warn('sendFriendRequest failed:', e);
      toast('Не удалось отправить: ' + (e.code || e.message || 'ошибка'));
    } finally {
  ...
  async function cancelFriendRequest(toUid) {
    ...
    try {
      ...
      if (document.getElementById('add-friend-modal')) filterAddFriend();
    } catch (e) { console.warn(e); toast('Не получилось отменить'); }
  }
  ...
  async function acceptFriendRequest(reqKey, fromUid, fromName) {
    ...
    try {
      await _db.ref().update(updates);
      toast('Теперь вы друзья 💗', 'var(--sage)');
    } catch (e) { console.warn(e); toast('Не получилось — попробуй ещё раз'); }
  }
  ...
  async function removeFriend(friendUid) {
    if (!confirm('Удалить друга? История чата сохранится.')) return;
    const me = myUid();
    const updates = {};
  ...
  async function sendChatMessage(otherUid, otherName, text) {
    text = (text || '').trim();
    if (!text) return;
    ...
    updates[`chats/${chatId}/messages/${newRef.key}`] = {
      from: me, fromName: myName(), text, at: now
    };
    ...
    // Mark as unread on the other side (timestamp of latest unread msg).
    updates[`users/${otherUid}/unreadFrom/${me}`] = now;
  ...
  async function markChatRead(otherUid) {
    const me = myUid();
    try { await _db.ref(`users/${me}/unreadFrom/${otherUid}`).remove(); } catch (_) {}
  }
  ...
  function attachFriendsListeners() {
    ...
    wire(`users/${me}/inbox`,    val => { _inboxCache    = val; renderFriendsAndChat(); updateChatBadges(); });
    wire(`users/${me}/outgoing`, val => { _outgoingCache = val; if (document.getElementById('add-friend-modal')) filterAddFriend(); });
    wire(`users/${me}/unreadFrom`, val => { _unreadFromCache = val; renderFriendsAndChat(); updateChatBadges(); });
  }

  function updateChatBadges() {
    // Update bottom nav profile badge or profile section header
    const requestsCount = Object.keys(_inboxCache || {}).length;
    const unreadCount = Object.keys(_unreadFromCache || {}).length;
    const total = requestsCount + unreadCount;
    let badge = document.getElementById('profile-nav-badge');
    const navItem = document.querySelector('.nav-item[data-screen="profile"]');
    if (!navItem) return;
    if (total > 0) {
      if (!badge) {
        badge = document.createElement('span');
        badge.id = 'profile-nav-badge';
  ...
  function friendRowHtml(uid, info) {
    ...
    const hasUnread = !!(_unreadFromCache && _unreadFromCache[uid]);
    const isAdmin = (_usersDirCache && _usersDirCache[uid] && _usersDirCache[uid].isAdmin);
    const avatar = (_usersDirCache && _usersDirCache[uid] && _usersDirCache[uid].avatar)
      ? `<div class="photo-zoom" style="width:42px;height:42px;border-radius:50%;background:url(${_usersDirCache[uid].avatar}) center/cover no-repeat;flex-shrink:0;border:2px solid var(--rose);"></div>`
      : `<div style="width:42px;height:42px;border-radius:50%;background:var(--paper);flex-shrink:0;border:2px solid var(--rose);display:flex;align-items:center;justify-content:center;font-size:16px;color:var(--coral);font-weight:700;">${escHtml(name.charAt(0).toUpperCase())}</div>`;
    return `
      <div class="card card-press" onclick="openChat('${escHtml(uid)}','${name.replace(/'/g,"&#39;")}')" style="display:flex;align-items:center;gap:12px;padding:10px 12px;cursor:pointer;">
        ${avatar}
        <div style="flex:1;min-width:0;">
          <div style="display:flex;align-items:baseline;gap:6px;">
            <span style="font-weight:600;color:var(--berry);font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${name}</span>
            ${isAdmin ? '<span class="chip chip-coral" style="font-size:9px;padding:1px 6px;">🎓 учитель</span>' : ''}
          </div>
          <div style="font-size:10.5px;color:var(--soft);margin-top:2px;">Тапни, чтобы открыть чат</div>
        </div>
        ${hasUnread ? '<span class="chip" style="background:var(--grad-coral);color:white;font-size:10px;flex-shrink:0;padding:3px 9px;">●</span>' : '<i class="fa-solid fa-comment" style="color:var(--coral);font-size:13px;flex-shrink:0;"></i>'}
      </div>`;
  }

  function requestRowHtml(reqKey, req) {
    const name = escHtml(req.fromName || 'Без имени');
    const fromUid = escHtml(req.from);
    return `
      <div class="card card-padded" style="display:flex;align-items:center;gap:10px;background:linear-gradient(160deg,#FFF4EE,white);">
        ...
        <div style="flex:1;min-width:0;">
          ...
          <div style="font-size:10.5px;color:var(--soft);">хочет добавить в друзья</div>
        </div>
        <div style="display:flex;gap:6px;flex-shrink:0;">
          <button onclick="acceptFriendRequest('${escHtml(reqKey)}','${fromUid}','${name.replace(/'/g,"&#39;")}')" class="btn btn-primary" style="padding:7px 12px;font-size:11px;">Принять</button>
  ...
  async function renderFriendsAndChat() {
    ...
    const friendUids = Object.keys(friends).sort((a, b) => (friends[b].since || 0) - (friends[a].since || 0));
    const requests = Object.entries(inbox).filter(([k, v]) => v && v.type === 'friend_request');

    const requestsHtml = requests.length
      ? `<div style="font-size:10px;letter-spacing:.18em;color:var(--coral);font-weight:700;margin:4px 4px 8px;">ЗАЯВКИ · ${requests.length}</div>
         <div style="display:grid;gap:8px;margin-bottom:14px;">${requests.map(([k,r]) => requestRowHtml(k, r)).join('')}</div>`
      : '';

    const friendsHtml = friendUids.length
      ? `<div style="display:grid;gap:8px;">${friendUids.map(uid => friendRowHtml(uid, friends[uid])).join('')}</div>`
      : `<div style="text-align:center;padding:18px 12px;color:var(--soft);font-size:12px;background:rgba(255,255,255,.55);border:1px dashed var(--line);border-radius:14px;">Пока никого в друзьях 🌸<br>Найди одногруппников или учителя ↓</div>`;

    slot.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:0 4px;margin-bottom:10px;">
        <div style="font-size:10px;letter-spacing:.2em;color:var(--soft);">ДРУЗЬЯ И ЧАТ ${friendUids.length ? `<span style="color:var(--coral);font-weight:700;letter-spacing:.04em;">${friendUids.length}</span>` : ''}</div>
        <button onclick="openAddFriendModal()" class="chip chip-coral" style="font-size:11px;padding:5px 12px;cursor:pointer;border:none;"><i class="fa-solid fa-user-plus" style="font-size:10px;"></i> Добавить</button>
      </div>
  ...
  function pushPermissionCtaHtml() {
    ...
    if (Notification.permission === 'denied') {
      return `<div style="background:rgba(232,155,161,.12);border:1px dashed rgba(232,155,161,.5);border-radius:14px;padding:10px 12px;margin-bottom:12px;font-size:11.5px;color:#B33A4A;text-align:center;">
        🔕 Уведомления заблокированы в браузере. Разреши их в настройках, чтобы получать сообщения в фоне.
      </div>`;
    }
    return `<div style="background:rgba(255,255,255,.7);border:1px solid var(--line);border-radius:14px;padding:12px;margin-bottom:12px;display:flex;align-items:center;gap:10px;">
      <div style="font-size:22px;">🔔</div>
      <div style="flex:1;min-width:0;">
        <div style="font-weight:600;color:var(--berry);font-size:12.5px;">Включи уведомления</div>
        <div style="font-size:10.5px;color:var(--soft);margin-top:1px;">Чтобы видеть новые сообщения даже когда сайт закрыт.</div>
      </div>
      <button onclick="askPushPermission()" class="btn btn-primary" style="padding:7px 12px;font-size:11px;flex-shrink:0;">Включить</button>
    </div>`;
  }
  ...
  function renderAdminChatList() {
    ...
    const me = myUid();
    const students = Object.entries(dir)
      .map(([uid, u]) => ({ uid, ...u }))
      .filter(u => !u.isAdmin && u.uid !== me)
      .sort((a, b) => {
        const aUnread = _unreadFromCache[a.uid] || 0;
        const bUnread = _unreadFromCache[b.uid] || 0;
        if (aUnread !== bUnread) return bUnread - aUnread;
        return (b.lastSeen || 0) - (a.lastSeen || 0);
      });

    const listHtml = students.length
      ? `<div style="display:grid;gap:8px;">${students.map(s => adminChatRowHtml(s)).join('')}</div>`
      : `<div style="text-align:center;padding:18px 12px;color:var(--soft);font-size:12px;background:rgba(255,255,255,.55);border:1px dashed var(--line);border-radius:14px;">Учеников пока нет 🌸</div>`;

    slot.innerHTML = `
      <div style="display:flex;justify-content:space-between;align-items:center;padding:0 4px;margin-bottom:10px;">
        <div style="font-size:10px;letter-spacing:.2em;color:var(--soft);">УЧЕНИКИ И ЧАТ <span style="color:var(--coral);font-weight:700;letter-spacing:.04em;">${students.length}</span></div>
        <button onclick="refreshAdminChatList()" class="chip" style="font-size:11px;padding:5px 12px;cursor:pointer;border:none;background:var(--blush);color:var(--berry);"><i class="fa-solid fa-rotate" style="font-size:10px;"></i> Обновить</button>
      </div>
      ${listHtml}
    `;
  }
  ...
  function adminChatRowHtml(u) {
    const name = escHtml(u.name || 'Без имени');
    const hasUnread = !!(_unreadFromCache && _unreadFromCache[u.uid]);
    const xp = (u.stats && u.stats.xp) || 0;
    const lvl = getLevel(xp);
    const rank = getRank(lvl);
    const initial = escHtml((u.name || '?').charAt(0).toUpperCase());
    const avatar = u.avatar
      ? `<div class="photo-zoom" style="width:42px;height:42px;border-radius:50%;background:url(${u.avatar}) center/cover no-repeat;flex-shrink:0;border:2px solid var(--rose);"></div>`
      : `<div style="width:42px;height:42px;border-radius:50%;background:var(--paper);flex-shrink:0;border:2px solid var(--rose);display:flex;align-items:center;justify-content:center;font-size:16px;color:var(--coral);font-weight:700;">${initial}</div>`;
    return `
      <div class="card card-press" onclick="openChat('${escHtml(u.uid)}','${name.replace(/'/g,"&#39;")}')" style="display:flex;align-items:center;gap:12px;padding:10px 12px;cursor:pointer;">
        ${avatar}
        <div style="flex:1;min-width:0;">
          <div style="display:flex;align-items:baseline;gap:6px;">
            <span style="font-weight:600;color:var(--berry);font-size:14px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">${name}</span>
            <span style="font-size:10.5px;color:var(--coral);">${rank.emoji} Lv ${lvl}</span>
          </div>
          <div style="font-size:10.5px;color:var(--soft);margin-top:1px;">${escHtml(u.email || '')} · 🔥 ${(u.stats && u.stats.streak) || 0} · ${xp} XP</div>
        </div>
        ${hasUnread ? '<span class="chip" style="background:var(--grad-coral);color:white;font-size:10px;flex-shrink:0;padding:3px 9px;">●</span>' : '<i class="fa-solid fa-comment" style="color:var(--coral);font-size:13px;flex-shrink:0;"></i>'}
      </div>`;
  }
      ...
      if (isFriend) {
        ...
        actionHtml = `
          <div style="display:flex;gap:5px;flex-shrink:0;align-items:center;">
            <span class="chip" style="font-size:10px;background:rgba(201,165,92,.18);color:#8A6A26;border:1px solid rgba(201,165,92,.5);">⏳ Отправлен</span>
            <button onclick="cancelFriendRequest('${escHtml(u.uid)}')" class="btn btn-ghost" style="padding:5px 9px;font-size:10.5px;border-color:rgba(232,155,161,.5);color:#B33A4A;">Отмена</button>
          </div>`;
      } else if (isSending) {
  ...
  function paintChatHeaderStatus() {
    ...
    const isOnline = _chatBuddyLastSeen && (Date.now() - _chatBuddyLastSeen) < ONLINE_WINDOW_MS;
  ...
  function openChat(otherUid, otherName) {
    if (!isLoggedInForChat()) { toast('Войди в аккаунт, чтобы открыть чат'); return; }
    const me = myUid();
    const chatId = chatIdFor(me, otherUid);
    ...
    const cached = (_usersDirCache && _usersDirCache[otherUid]) || {};
    const initialAvatar = cached.avatar || '';
    const initialLastSeen = cached.lastSeen || 0;
    _chatBuddyLastSeen = initialLastSeen;
    const avatarStyle = initialAvatar
      ? `background:url(${initialAvatar}) center/cover no-repeat;`
      : '';
    const avatarContent = initialAvatar ? '' : escHtml((otherName || '?').charAt(0).toUpperCase());
    const avatarClass = initialAvatar ? 'chat-buddy-avatar photo-zoom' : 'chat-buddy-avatar';
    const avatarData = initialAvatar ? `data-photo-src="${initialAvatar}"` : '';
    m.innerHTML = `
      <div class="chat-sheet">
        <div class="chat-header">
          <button id="chat-close-btn" class="chat-back-btn" aria-label="Назад"><i class="fa-solid fa-arrow-left"></i></button>
          <div class="chat-buddy-info">
            <div class="chat-buddy-name">${escHtml(otherName)}</div>
            <div id="chat-buddy-status" class="chat-buddy-status">${chatStatusHtml(initialLastSeen)}</div>
          </div>
          ${showRemoveBtn ? '<button id="chat-remove-friend-btn" class="chat-mini-btn" aria-label="Удалить из друзей" title="Удалить из друзей"><i class="fa-solid fa-user-xmark"></i></button>' : ''}
          <div id="chat-buddy-avatar" class="${avatarClass}" ${avatarData} style="${avatarStyle}">${avatarContent}</div>
        </div>
        <div class="chat-body" id="chat-body">
          <div style="text-align:center;padding:24px;color:var(--soft);font-size:12px;">Загружаю…</div>
        </div>
    ...
    paintChatHeaderStatus();
    // Subscribe to live updates of avatar + lastSeen
    attachChatBuddyListener(otherUid);

    // Event listeners (avoid inline onclick — safer with names containing apostrophes etc.)
    document.getElementById('chat-close-btn').addEventListener('click', closeChat);
    if (showRemoveBtn) {
      document.getElementById('chat-remove-friend-btn').addEventListener('click', () => removeFriend(otherUid));
    }
    ...
    form.addEventListener('submit', async (e) => {
      ...
      try {
        ...
        toast('Сообщение не отправилось: ' + (err.message || err.code || 'ошибка'));
  ...
  function attachChatListener(chatId) {
    detachChatListener();
    if (typeof _db === 'undefined') return;
    const ref = _db.ref(`chats/${chatId}/messages`).limitToLast(200);
    const handler = snap => {
      const msgs = snap.val() || {};
      renderChatMessages(msgs);
    };
    ref.on('value', handler);
    _chatListenerOff = () => ref.off('value', handler);
  }
  ...
  function renderChatMessages(msgs) {
    ...
    if (!entries.length) {
      body.innerHTML = `<div style="text-align:center;padding:36px 24px;color:var(--soft);font-size:12px;">Пока тут пусто 🌸<br>Напиши первое сообщение!</div>`;
      return;
    }
    // Group by day
    let lastDay = '';
    ...
    for (const m of entries) {
      ...
      if (dayKey !== lastDay) {
        const label = formatChatDay(d);
        html += `<div class="chat-day-label"><span>${escHtml(label)}</span></div>`;
        lastDay = dayKey;
      }
  ...
  async function askPushPermission() {
    if (!_messaging) { toast('Этот браузер не поддерживает push 🌸'); return; }
    if (!('Notification' in window)) { toast('Этот браузер не поддерживает уведомления'); return; }
    if (!_VAPID_KEY || _VAPID_KEY.startsWith('REPLACE_')) {
      toast('Push ещё не настроен (нет VAPID ключа)');
      return;
    }
    if (Notification.permission === 'granted') {
      toast('Уведомления уже включены ✓', 'var(--sage)');
      return;
    }
    if (Notification.permission === 'denied') {
      toast('Разреши уведомления в настройках браузера');
      return;
    }
    ...
    await registerFcmToken(swReg);
    attachForegroundMessageHandler();
    toast('Push-уведомления включены 🔔', 'var(--sage)');
  }
  ...
  function attachForegroundMessageHandler() {
    ...
    _pushUnsubFg = _messaging.onMessage(payload => {
      ...
      const title = noti.title || data.fromName || 'Новое сообщение';
      ...
      // In-app toast (a quick visual ping even if OS notification can't render)
      try { toast(`💬 ${title}: ${body.length > 60 ? body.slice(0, 60) + '…' : body}`, 'var(--berry)'); } catch (_) {}
      ...
      <div onclick="startLessonFlow()" class="hero" style="position:relative;">
        ...
        <div style="margin-top:20px;">
          <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:6px;">
            ...
            <span style="color: rgba(92,42,51,.65); font-weight:800;">${pct}%</span>
          </div>
  ...
  // After a touch-handled double-tap, mobile browsers still synthesize a dblclick from the two
  // clicks — we suppress that duplicate via a short-lived timestamp.
  const _mediaTapTimes = {};
  let _touchDoubleTapAt = 0;
  ...
  function doubleTapLike(pid, wrap, ev) {
    // Suppress dblclick that mobile browsers synthesize right after we already handled
    // the double-tap from touchend — otherwise we'd spawn two hearts.
    if (ev && ev.type === 'dblclick' && Date.now() - _touchDoubleTapAt < 700) return;
  ...
  // Global tap-to-enlarge for any element with .photo-zoom
  // Extracts URL from: data-photo-src → inline backgroundImage → inner <img>.
  // Capture phase + stopPropagation so clicking an avatar inside a friend card
  // opens the lightbox instead of the chat.
  document.addEventListener('click', e => {
    const el = e.target.closest('.photo-zoom');
    if (!el) return;
    let src = el.dataset.photoSrc || '';
    if (!src) {
  ...
  function renderCommentItem(pid, c) {
    ...
    const avClass = avUrl ? 'feed-comment-avatar photo-zoom' : 'feed-comment-avatar';
    const own = c.uid === me;
  ...
  function addXp(n, label) {
    ...
    const wasReached = stats.dailyXp >= goal;
    stats.dailyXp += n;
    ...
    if (!wasReached && nowReached) triggerDailyGoalCelebration();
  }
  ...
  function ensureDailyCounter() {
    ...
    if (stats.dailyDate !== today) {
      ...
      stats.dailyXp = 0;
      if (!Array.isArray(stats.dailyGoalReachedDates)) stats.dailyGoalReachedDates = [];
      UStore.set('stats', stats);
    }
    if (!stats.dailyGoal || stats.dailyGoal < 5) stats.dailyGoal = 30;
  }

  function renderDailyGoal() {
    ensureDailyCounter();
    const slot = document.getElementById('daily-goal-widget');
    if (!slot) return;
    const cur = stats.dailyXp || 0;
    const goal = stats.dailyGoal || 30;
    ...
    slot.innerHTML = `
      <div onclick="openDailyGoalSettings()" class="card card-press" style="padding:14px 16px; cursor:pointer; ${done ? 'background:linear-gradient(135deg, rgba(229,180,60,.20), rgba(255,255,255,.7));' : ''}">
        <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:8px;">
          ...
          <span style="font-size:13px; font-weight:800; color:${done ? '#A37A1A' : 'var(--coral)'}; flex-shrink:0;">${cur}/${goal} XP</span>
        </div>
        <div style="height:10px; background:var(--paper); border-radius:999px; overflow:hidden;">
          <div style="height:100%; width:${pct}%; background:${done ? 'linear-gradient(90deg, #E5B43C, #C9A55C)' : 'var(--grad-coral)'}; border-radius:999px; transition:width .5s ease;"></div>
        </div>
  ...
  function openDailyGoalSettings() {
    ...
    m.onclick = e => { if (e.target === m) m.remove(); };
    const options = [
      { xp: 15,  label: 'Лёгкая',     desc: '5 минут в день' },
      { xp: 30,  label: 'Обычная',    desc: '10-15 минут' },
      { xp: 50,  label: 'Серьёзная',  desc: '20-30 минут' },
      { xp: 100, label: 'Интенсивно', desc: '~1 час в день' }
    ];
    const cur = stats.dailyGoal || 30;
    m.innerHTML = `
      <div class="modal-card" style="max-width:380px;">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:14px;">
          <div>
            <div class="page-eyebrow">ДНЕВНАЯ ЦЕЛЬ</div>
            <div class="display" style="font-size:20px; color:var(--berry); margin-top:2px;">Сколько XP в день?</div>
          </div>
        ...
        <div style="display:grid; gap:8px;">
          ${options.map(o => `
            <div onclick="setDailyGoal(${o.xp}); this.closest('.modal-bg').remove();" class="card card-press" style="padding:12px 14px; cursor:pointer; ${o.xp === cur ? 'border:2px solid var(--coral); background:rgba(232,155,161,.08);' : ''}">
              <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                  <div style="font-weight:600; color:var(--berry); font-size:14px;">${o.label} · ${o.xp} XP</div>
                  <div style="font-size:11px; color:var(--soft); margin-top:2px;">${o.desc}</div>
                </div>
                ${o.xp === cur ? '<span style="color:var(--coral); font-size:16px;">✓</span>' : ''}
              </div>
            </div>
          `).join('')}
        </div>
  ...
  function setDailyGoal(xp) {
    ...
    syncStats();
    renderDailyGoal();
    toast(`Цель: ${xp} XP в день 🎯`, 'var(--sage)');
  }

  function triggerDailyGoalCelebration() {
    // Mark today as reached
    const today = todayIsoDate();
    if (!Array.isArray(stats.dailyGoalReachedDates)) stats.dailyGoalReachedDates = [];
    if (!stats.dailyGoalReachedDates.includes(today)) {
    ...
    m.innerHTML = `
      <div class="modal-card lesson-complete-card">
        ...
        <div class="page-eyebrow" style="color:var(--coral);">ЦЕЛЬ ДНЯ ✓</div>
        <div class="display" style="font-size:24px; color:var(--berry); margin-top:6px; line-height:1.15;">${stats.dailyGoal} XP за день!</div>
    ...
    document.body.appendChild(m);
    setTimeout(() => { document.getElementById('daily-goal-popup')?.remove(); }, 6000);
  }

  function spawnConfetti() {
    const layer = document.createElement('div');
    layer.style.cssText = 'position:fixed; inset:0; pointer-events:none; z-index:950; overflow:hidden;';
    const colors = ['#E07686', '#C9A55C', '#84C474', '#E89BA1', '#D4AAB0'];
    for (let i = 0; i < 60; i++) {
      ...
      c.style.cssText = `position:absolute; top:-20px; left:${left}%; width:${size}px; height:${size * 1.4}px; background:${colors[i % colors.length]}; border-radius:2px; opacity:.95; transform:rotate(${rot}deg); animation:confettiFall ${dur}s ${delay}s cubic-bezier(.2,.7,.4,1) forwards;`;
      layer.appendChild(c);
    }
  ...
  function showStudentDetail(uid) {
    ...
    const initial = (u.name || '?').charAt(0).toUpperCase();
    const avatar = u.avatar
      ? `<div class="photo-zoom" style="width:96px; height:96px; border-radius:50%; background:url(${u.avatar}) center/cover no-repeat; border:3px solid var(--rose); box-shadow: var(--shadow-md);"></div>`
  ...
  function setActiveUser(user) {
    ...
    // Ensure profile (incl. admin) is discoverable in chat search.
    pushUserToCloud();
    // Re-render feed social bars so "liked by me" reflects the new identity
    refreshFeedSocial();
    // Push notifications: re-init for the new user (gets fresh FCM token + saves under their uid)
    _pushInitDone = false;
  ...
  function logoutUser() {
    ...
    detachFriendsListeners();
    detachChatListener();
    _friendsCache = {}; _inboxCache = {}; _unreadFromCache = {};
    Store.del('user');
  ...
  function wordHeartBtn(ko, ru, translit, emoji, source) {
    ...
    const icon = saved
      ? '<i class="fa-solid fa-heart" style="color:#E07686;"></i>'
      : '<i class="fa-regular fa-heart" style="color:rgba(92,42,51,.45);"></i>';
    ...
    return `<button
      ...
      style="position:absolute; top:6px; right:6px; width:26px; height:26px; border-radius:50%; border:none; background:rgba(255,255,255,.85); cursor:pointer; padding:0; display:flex; align-items:center; justify-content:center; font-size:11px; line-height:1; box-shadow:0 2px 0 rgba(92,42,51,.08); z-index:2;">${icon}</button>`;
  }

  // ── Game shell (reusable modal) ──
  function gameModal(html, opts) {
    const id = (opts && opts.id) || 'game-modal';
    const existing = document.getElementById(id);
    // If the modal is already open, just swap the card content in place.
    // Replacing the whole node re-fires the fadeIn/sheetUp animations and
    // causes a visible flicker on every interaction (memory game etc.).
    if (existing) {
      const card = existing.querySelector('.modal-card');
      if (card) { card.innerHTML = html; return existing; }
    }
    const m = document.createElement('div');
    m.className = 'modal-bg modal-center';
    m.id = id;
  ...
  function renderMemory() {
    ...
    const cardsHtml = memCards.map(c => {
      ...
      const baseStyle = 'aspect-ratio:1; display:flex; align-items:center; justify-content:center; cursor:pointer; padding:6px; text-align:center; min-height:62px; position:relative;';
      const matchedStyle = c.matched ? ' opacity:.75; background:rgba(229,180,60,.18) !important; border-color:var(--duo-gold) !important;' : '';
      const hiddenStyle = !visible ? ' background:linear-gradient(140deg, var(--coral), var(--rose)) !important; color:white; border-color:transparent !important;' : '';
      // When the card is revealed, show the matching pair from memoryData so we can save BOTH ko and ru
      const pair = memoryData.find(p => p.ko === c.text || p.ru === c.text);
      const koOfPair = pair ? pair.ko : '';
      const ruOfPair = pair ? pair.ru : '';
      const heart = (visible && koOfPair) ? wordHeartBtn(koOfPair, ruOfPair, '', '🌸', 'memory') : '';
      ...
      return `<div onclick="flipMemoryCard(${c.idx})" class="card card-press" style="${baseStyle}${matchedStyle}${hiddenStyle}">${heart}${content}</div>`;
    }).join('');
    gameModal(`
  ...
  document.addEventListener('visibilitychange', () => { if (!document.hidden) bumpLastSeen(); });
